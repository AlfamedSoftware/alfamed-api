...
export const createSessionRoutes = (db: DatabaseClient) => {
    ...
    return new Elysia({ name: "session-routes", prefix: "/session" })
        ...
        /**
         * POST /session/switch-unit
         * Muda a unidade selecionada (para usuários com múltiplas unidades)
         * Mesmo que select-unit mas com semântica de "trocar"
         */
        .post(
            "/switch-unit",
            async (context) => {
                ...
                return status(200, {
                    success: true,
                    message: "Unit switched successfully. Please refresh your session.",
                } satisfies SelectUnitResponse);
            },
            {
                body: t.Object({
                ...
                detail: {
                    summary: "Switch to a different unit",
                    description:
                        "Changes the active unit in the current session. Validates access before allowing the switch.",
                    tags: ["Session Management"],
                },
                response: {
                    200: t.Object({
